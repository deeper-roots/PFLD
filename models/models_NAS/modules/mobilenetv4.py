...
__all__ = ['mobilenetv4_conv_small', 'mobilenetv4_conv_medium', 'mobilenetv4_conv_large',
           'mobilenetv4_hybrid_medium', 'mobilenetv4_hybrid_large','UniversalInvertedBottleneck']
...
class MobileNetV4_bockbone_fixed(nn.Module):
    def __init__(self, block_specs, num_classes=196):
        super(MobileNetV4_bockbone_fixed, self).__init__()
        c = 3
        layers = []
        for block_type, *block_cfg in block_specs:
            if block_type == 'conv_bn':
...
def mobilenetv4_conv_small_bockbone_fixed(**kwargs):
    """
    Constructs a MobileNetV4-Conv-Small model
    """
    block_specs = [
...
#  return MobileNetV4(block_specs, **kwargs)
    return MobileNetV4_bockbone_fixed(block_specs, **kwargs)
